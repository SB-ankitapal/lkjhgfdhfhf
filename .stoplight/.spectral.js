...
var checkTimezoneHeaderParam = createRulesetFunction(
  {
    input: {
      type: 'object',
    },
...
var require200IfDelete = createRulesetFunction(
  {
    input: {
      type: 'object',
    },
...
var require200or201IfPost = createRulesetFunction(
  {
    input: {
      type: "object"
    },
...
var require400IfPostWithBody = createRulesetFunction(
  {
    input: {
      type: 'object',
    },
...
var require401IfJwt = createRulesetFunction(
  {
    input: {
      type: "object"
    },
...
var require404IfGetWithPathParams = createRulesetFunction(
  {
    input: {
      type: "object"
    },
...
var requireJwtSecurity = createRulesetFunction(
  {
    input: {
      type: "object"
    },
...
// .stoplight/custom-functions/validateSchemaIfPresent.js

var schemaIfPresent = createRulesetFunction(
  {
    input: {
  ...
  async function schemaIfPresent(targetVal, opts, context) {
    ...
    return schema(targetVal, opts, context);
  }
);
...
function findMatchingSchemaNames$3(allSchemas, candidateSchema) {
  return Object.entries(allSchemas)
    .filter(([_, schema]) => isEqualSchema$3(schema, candidateSchema))
    .map(([name]) => name);
}

// Determina si la regla se aplica
function applies$3(schemaName, schemaObject) {
...
var validateDobleLista = createRulesetFunction(
  {
    input: {
      type: "object"
    },
  ...
  function validateDobleLista(schemaObject, _opts, context) {
    ...
    // Salir si no aplica
    if (!applies$3(schemaName, schemaObject)) {
      return;
    }
    ...
    if (listadoInsertarItemsSchema?.type === "string") {
      ...
      insertarNames.push(...findMatchingSchemaNames$3(allSchemas, listadoInsertarItemsSchema));
    } else {
      errors.push({
...
// Devuelve una lista de nombres de esquemas que coincidan estructuralmente
function findMatchingSchemaNames$2(allSchemas, candidateSchema) {
  return Object.entries(allSchemas)
    .filter(([_, schema]) => isEqualSchema$2(schema, candidateSchema))
    .map(([name]) => name);
}

// Determina si la regla se aplica
function applies$2(schemaName, schemaObject) {
  const props = schemaObject?.properties ?? {};
  const keys = Object.keys(props);
...
var validateDobleListaObjeto = createRulesetFunction(
  {
    input: {
      type: 'object',
    },
  ...
  function validateDobleListaObjeto(schemaObject, _opts, context) {
    ...
    // Salir si no aplica
    if (!applies$2(schemaName, schemaObject)) {
      return;
    }
    ...
    if (listadoInsertarItemsSchema?.type === "string") {
      ...
      insertarNames.push(...findMatchingSchemaNames$2(allSchemas, listadoInsertarItemsSchema));
    } else {
      errors.push({
        message: "Tipo inválido para array de 'listadoInsertar': " + listadoInsertarItemsSchema?.type,
        path: [...context.path, 'properties', 'listadoInsertar', 'items'],
    ...
    if (listadoEliminarItemsSchema?.type === "string") {
      ...
      eliminarNames.push(...findMatchingSchemaNames$2(allSchemas, listadoEliminarItemsSchema));
    } else {
      errors.push({
...
var validateOperationId = createRulesetFunction(
  {
    input: {
      type: "object"
    },
...
var validateResponseDescription = createRulesetFunction(
  {
    input: {
      type: 'object',
    },
...
// Devuelve una lista de nombres de esquemas que coincidan estructuralmente
function findMatchingSchemaNames$1(allSchemas, candidateSchema) {
  return Object.entries(allSchemas)
    .filter(([_, schema]) => isEqualSchema$1(schema, candidateSchema))
    .map(([name]) => name);
}

// Determina si la regla se aplica
function applies$1(schemaName, schemaObject) {
  const props = schemaObject?.properties ?? {};
  const keys = Object.keys(props);
...
var validateTripleLista = createRulesetFunction(
  {
    input: {
      type: "object"
    },
  ...
  function validateTripleLista(schemaObject, _opts, context) {
    ...
    // Salir si no aplica
    if (!applies$1(schemaName, schemaObject)) {
      return;
    }
    ...
    if (listadoInsertarItemsSchema?.type === "string") {
      ...
      insertarNames.push(...findMatchingSchemaNames$1(allSchemas, listadoInsertarItemsSchema));
    } else {
      errors.push({
        message: "Tipo inválido para array de 'listadoInsertar': " + listadoInsertarItemsSchema?.type,
        path: [...context.path, 'properties', 'listadoInsertar', 'items'],
    ...
    if (listadoModificarItemsSchema?.type === "string") {
      ...
      modificarNames.push(
        ...findMatchingSchemaNames$1(allSchemas, listadoModificarItemsSchema)
      );
    } else {
      errors.push({
        message:
...
var validateTripleListaObjeto = createRulesetFunction(
  {
    input: {
      type: "object"
    },
      ...
      "then": {
        ...
        "functionOptions": {
          "schema": {
            ...
            "items": {
              "type": "string",
              "enum": ["activar_usuario", "superadministrador", "no_administrador", "calendario_publico", "dios", "empleado_externo", "empleado_interno", "empresa_externo", "empresa_interno", "firmar_informacion", "reset_password", "reconocimiento_medico", "firma_pantalla_publica", "administrador_grupo_entornos"]
            }
          }
        },
        "field": "x-roles"
